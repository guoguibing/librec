 ...
 * @author Keqiang Wang
 */
public class MPEEvaluator extends AbstractRecommenderEvaluator<DoubleCollection> {



    private double mpe;

    public MPEEvaluator(double mpe) {
        this.mpe = mpe;
    }
    ...
    @Override
    public double evaluate(DoubleCollection ItemIndexSet, List<KeyValue> recommendedList){
        float mpe = 0F;
        Iterator<Double> iterator = ItemIndexSet.iterator();
        for (KeyValue<Integer, Double> keyValue : recommendedList) {
