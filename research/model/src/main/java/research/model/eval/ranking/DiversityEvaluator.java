 ...
 * @author Keqiang Wang
 */
public class DiversityEvaluator extends AbstractRecommenderEvaluator<IntCollection> {

    private SymmMatrix similarityMatrix;

    public DiversityEvaluator(SymmMatrix similarityMatrix) {
            ...
            if (recommendArrayListByContext.size() > 1) {
                ...
                for (int indexOut = 0; indexOut < topK; ++indexOut) {
                    for (int indexIn = 0; indexIn < topK; ++indexIn) {
                        ...
                        totalDisSimilarityPerContext += 1.0 - similarityMatrix.get(keyOut, keyIn);
                    }
                }
    ...
    /**
     *  Evaluate diversity for the single user(u).
     *  D_{u} = 1 - \frac{2\sum_{i,j\in R(u)}s(i,j)}{|R(u)||R(u) - 1|}
     *  D_{u} is  recommend list  for the single user u.
     * @param itemIdSetInTest
     * @param recommendedList  D_{u} is  recommend list  for the single user u.
     * @return evaluate result
     */
    ...
    public double evaluate(IntCollection itemIdSetInTest, List<KeyValue> recommendedList){
        ...
        for(int i = 0; i < TopK; i++){
            for(int j = i+1; j < TopK; j++){
                int key1 =  (int)recommendedList.get(i).getKey(), key2 = (int) recommendedList.get(j).getKey();
                similarity += (this.similarityMatrix.get(key1, key2) + this.similarityMatrix.get(key1, key2));
                }
        }
        return 1 -  2 * similarity / (TopK * (TopK + 1));
    }
