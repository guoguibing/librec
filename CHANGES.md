...
* Class improvements
  ...
  * Sparse Matrix
    ...
    * New methods `reshape` added to remove zero entries and reshape dimensions
    * New method `matString` added to print in matrix-like format, useful when dimension is small for debug purpose
  ...
  * `LineConifer` is added to encapsulate the line of parameter with options 
* LibRec commands & configuration
  ...
  * New entries added for probabilistic graphic models
    * num.burn.in
    * num.sample.lag
    * num.verbose.interval
    * val.init.alpha/beta
  * `evaluation.setup` is devised to substitute the following old entries
    * is.cross.validation
    * is.parallel.folds
    * num.kfold
    * val.ratio
