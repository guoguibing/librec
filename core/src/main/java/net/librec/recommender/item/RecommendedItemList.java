...
public class RecommendedItemList implements RecommendedList, Serializable {
    ...
    public RecommendedItemList(int maxUserIdxParam, int initCapacityParam) {
        ...
        this.elementData = new ArrayList<>(initCapacityParam);
    ...
    public boolean addUserItemIdx(int userIdx, int itemIdx, double rating) {
        userRangeCheck(userIdx);
        checkIndex(userIdx);
        elementData.get(indexOfUserIdx[userIdx]).add(new ItemEntry<>(itemIdx, rating));
        return true;
    }
    ...
    public List<ItemEntry<Integer, Double>> removeUserIdx(int userIdx) {
        userRangeCheck(userIdx);
        int index = indexOfUserIdx[userIdx];
        List<ItemEntry<Integer, Double>> oldValue = new ArrayList<>();
        if (index > 0) {
