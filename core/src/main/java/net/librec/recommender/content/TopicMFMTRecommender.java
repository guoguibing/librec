 ...
 * @author SunYatong
 */
public class TopicMFMTRecommender extends TensorRecommender {
                ...
                for (int factorIdx = 0; factorIdx < numTopics; factorIdx++) {
                    ...
                    for (Vector.VectorEntry ve : wordVec) {
                        ...
                        for (int topicIdx = 0; topicIdx < numTopics; topicIdx++) {
                            ...
                            //update K1 K2
                            K += learnRate * lambda * w_error * phi.get(topicIdx, wordIdx) * theta.get(documentId, topicIdx) * (1 - theta.get(documentId, topicIdx)) * Math.abs(userFactors.get(i, topicIdx));
                        }
                        userFactors.plus(i, factorIdx, learnRate * K * derivative * itemFactors.get(j, factorIdx));
                        itemFactors.plus(j, factorIdx, learnRate * K * derivative * userFactors.get(i, factorIdx));
                    }
                }
            }
            // calculate theta
    ...
    private void calculateTheta() {
        for (MatrixEntry me : trainMatrix) {
            ...
            for (int k = 0; k < numTopics; k++) {
                k1uAddk2v[k] = K * Math.abs(userFactors.get(u, k)) * Math.abs(itemFactors.get(i, k));
            }
