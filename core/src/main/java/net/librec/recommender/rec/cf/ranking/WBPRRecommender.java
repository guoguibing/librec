...
public class WBPRRecommender extends MatrixFactorizationRecommender {
    ...
    protected void setup() throws LibrecException {
        ...
        for (int itemIdx = 0; itemIdx < numItems; itemIdx++) {
            sortedItemPops.add(new AbstractMap.SimpleEntry<Integer, Double>(itemIdx, Double.valueOf(trainMatrix.columnSize(itemIdx))));
        }
    ...
    protected void trainModel() throws LibrecException {
        ...
        for (int iter = 1; iter <= numIterations; iter++) {
            for (int sampleCount = 0, smax = numUsers * 100; sampleCount < smax; sampleCount++) {
                ...
                double cmg = g(-diffValue);

                // update bias
                double posItemBiasValue = itemBiases.get(posItemIdx), negItemBiasValue = itemBiases.get(negItemIdx);
                itemBiases.add(posItemIdx, learnRate * (cmg - regBias * posItemBiasValue));
                itemBiases.add(negItemIdx, learnRate * (-cmg - regBias * negItemBiasValue));
                ...
                for (int factorIdx = 0; factorIdx < numFactors; factorIdx++) {
                    ...
                    userFactors.add(userIdx, factorIdx, learnRate * (cmg * (posItemFactorValue - negItemFactorValue) - regUser * userFactorValue));
                    itemFactors.add(posItemIdx, factorIdx, learnRate * (cmg * userFactorValue - regItem * posItemFactorValue));
                    itemFactors.add(negItemIdx, factorIdx, learnRate * (cmg * (-userFactorValue) - regItem * negItemFactorValue));
