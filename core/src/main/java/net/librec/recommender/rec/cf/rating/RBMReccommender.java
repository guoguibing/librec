...
public class RBMReccommender extends MatrixFactorizationRecommender {
	...
	public RBMReccommender(SparseMatrix rm, SparseMatrix tm, int fold) {
		...
		momentum = conf.getDouble("-m", 0);
		lamtaw = conf.getDouble("-lw", 0.001);
		lamtab = conf.getDouble("-lb", 0);
	...
	protected void setup() throws LibrecException {
		...
		for (int u = 0; u < numUsers; u++) {
			...
			for (int j = 0; j < num; j++) {
				...
				int r = (int) trainMatrix.get(u, m) - 1;
				moviecount[m][r]++;
			}
		}
	...
	protected double predict(int u, int m) throws LibrecException {
		...
		for (int i = 0; i < trainNumber; i++) {
			...
			int rate = (int) trainMatrix.get(u, item) - 1;
