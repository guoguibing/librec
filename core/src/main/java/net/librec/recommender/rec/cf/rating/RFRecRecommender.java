...
public class RFRecRecommender extends MatrixFactorizationRecommender {
	...
	public RFRecRecommender(SparseMatrix trainMatrix, SparseMatrix testMatrix, int fold) {
	...
	protected void setup() throws LibrecException {
		...
		for (MatrixEntry matrixEntry : trainMatrix) {
			...
			userRatingFrequencies.add(userIdx, realRating - 1, 1);
			itemRatingFrequencies.add(itemIdx, realRating - 1, 1);
		}
		userWeights = new DenseVector(numUsers);
		itemWeights = new DenseVector(numItems);
	}
	...
	public double predict(int userIdx, int itemIdx) {
		...
		if (userRatingFrequencies.row(userIdx).sum() > 0 && itemRatingFrequencies.row(itemIdx).sum() > 0
			...
			for (int r = 0; r < ratingScale.size(); ++r) {
				...
				// user component
				int tmpUser = 0;
				double frequencyInt = userRatingFrequencies.get(userIdx, ratingValue - 1);
				...
				int tmpItem = 0;
				frequency = 0;
				frequencyInt = itemRatingFrequencies.get(itemIdx, ratingValue - 1);
