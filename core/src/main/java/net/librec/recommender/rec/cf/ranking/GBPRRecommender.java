...
public class GBPRRecommender extends MatrixFactorizationRecommender {
    ...
    protected void setup() throws LibrecException {
        ...
        rho = conf.getFloat("rec.GBPR.rho");
        gLen = conf.getInt("rec.GBPR.gSize");
    ...
    protected void trainModel() throws LibrecException {
        for (int iter = 1; iter <= numIterations; iter++) {
            ...
            for (int sample = 0, smax = numUsers * 100; sample < smax; sample++) {
                ...
                itemBiases.add(posItemIdx, learnRate * (cmg - regBias * posBiasValue));

                double negBiasValue = itemBiases.get(negItemIdx);
                itemBiases.add(negItemIdx, learnRate * (-cmg - regBias * negBiasValue));
                ...
                for (int groupUserIdx : groupSet) {
                    ...
                    for (int factorIdx = 0; factorIdx < numFactors; factorIdx++) {
                        ...
                        tempUserFactors.add(groupUserIdx, factorIdx, learnRate * (cmg * deltaGroup - regUser * groupUserFactorValue));
                ...
                for (int factorIdx = 0; factorIdx < numFactors; factorIdx++) {
                    ...
                    tempItemFactors.add(posItemIdx, factorIdx, learnRate * (cmg * posDelta - regItem * posItemFactorValue));

                    double negDelta = -userFactorValue;
                    tempItemFactors.add(negItemIdx, factorIdx, learnRate * (cmg * negDelta - regItem * negItemFactorValue));
                }
            }
