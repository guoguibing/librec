 ...
 */
@ModelData({"isRanking", "knn", "userMeans", "trainMatrix"})
public class UserKNNRecommender extends AbstractRecommender {
    ...
    protected void setup() throws LibrecException {
        ...
        for (int userIdx = 0; userIdx < numUsers; ++userIdx) {
            ...
            userSimilarityList[userIdx] = new ArrayList<>(similarityVector.size());
            Iterator<VectorEntry> simItr = similarityVector.iterator();
            while (simItr.hasNext()) {
                VectorEntry simVectorEntry = simItr.next();
                userSimilarityList[userIdx].add(new AbstractMap.SimpleImmutableEntry<>(simVectorEntry.index(), simVectorEntry.get()));
            }
            Lists.sortList(userSimilarityList[userIdx], false);
        }
    }
