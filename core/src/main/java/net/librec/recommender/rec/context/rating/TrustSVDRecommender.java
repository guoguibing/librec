        ...
        for (int iter = 1; iter <= numIterations; iter++) {
            ...
            for (MatrixEntry matrixEntry : trainMatrix) {
                ...
                for (int factorIdx = 0; factorIdx < numFactors; factorIdx++) {
                    ...
                    double deltaItem = error * (userFactorValue + sumImpItemsFactors[factorIdx] + sumTrusteesFactors[factorIdx]) + regItem * itemWeight * itemFactorValue;

                    tempUserFactors.add(userIdx, factorIdx, deltaUser);
                    itemFactors.add(itemIdx, factorIdx, -learnRate * deltaItem);

                    for (int impItemIdx : impItemsList) {
                        double impItemValue = impItemFactors.get(impItemIdx, factorIdx);

                        double impItemWeightValue = impItemWeights.get(impItemIdx);
                        double deltaImpItem = error * itemFactorValue / userWeightDenom + regItem * impItemWeightValue * impItemValue;
            ...
            for (MatrixEntry socialMatrixEntry : socialMatrix) {
                ...
                double csgd = regSocial * socialError;
                ...
                for (int factorIdx = 0; factorIdx < numFactors; factorIdx++) {
                    ...
                    tempUserFactors.add(userIdx, factorIdx, csgd * trusteeFactorValue + regSocial * trusterWeightValue * userFactorValue);
                    trusteeTempFactors.add(trusteeIdx, factorIdx, csgd * userFactorValue);
