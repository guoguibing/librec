...
/**
 * underestiamtion Unfairness Evaluator is based on the method proposed by
 ...
 * is important where missing recommendations are more critical than extra recommendations.
 *
 * @author Nasim Sonboli
 */


public class UnderestimationUnfairnessEvaluator extends AbstractRecommenderEvaluator {
...
//            Set<Integer> testSetByUser = testMatrix.getColumnsSet(userID);
        ...
        for (int indexOfItem = 0; indexOfItem < numItems; indexOfItem++) {
            ...
            underEstimationUnfairness += Math.abs(Math.max(0.0, avgRatingPro - avgRecPro) - Math.max(0.0, avgRatingUnpro - avgRecUnpro));
        }
        // taking average over all the items
        underEstimationUnfairness /= numItems;
        return underEstimationUnfairness;
    }
}
