 ...
 * @author Zhang Chaoli
 */
public class ItemKNNJavaTestCase {

    public static void main(String[] args) {

        LibrecConf conf = new LibrecConf().setMaster("local[*]").setAppName("ItemKNNJava");
        ...
        RDD<IndexedVector> indexedVectorRDD = StatisticalFunctions.toIndexedSparseVectors(data, "item");
        RDD similarity = (new SimilarityFunctions(indexedVectorRDD)).computeSimilarity(Correlation.BCOS(), conf);
        ItemKNN itemKNN = new ItemKNN(10, true, trainData, similarity);
        itemKNN.train();

//        JavaConverters.asScalaBufferConverter(predictArr).asScala().toSeq();
        RDD<Rating> predictResult = itemKNN.predict(testData);
